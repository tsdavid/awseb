...
def usj():
	...
	for key in prdData.keys():
		...
		if len(st) <= 4: pass
		try:
			data[st['code2']] = json.loads(ut.download(apiUrl + st['code2']))
		except:
			pass
	...
	today = ut.dateformatter(now.year, now.month, now.day)
	send_dic = {}
	...
	for par_key in data.keys():
		...
		for key in dic.keys():
			for i in range(dic[key]):  # 각 월의 일 수를 늘린다음에
				y_m_d = ut.dateformatter(now.year, key, i + 1)  # 날짜 데이터 형식으로 저장하고
				price_dic[y_m_d] = '매진'  # 모두 매진이라 표기한 뒤 저장한다.

		# 가격을 가지고 오자
		lastest_day = ut.dateformatter(obj[-1]['year'], obj[-1]['month'], obj[-1]['date'])

		for o in obj:
			ymd = ut.dateformatter(o['year'], o['month'], o['date'])
			price = o['price1value']
